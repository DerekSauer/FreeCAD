...
Py::Object View3DInventorViewerPy::setGradientBackgroundColor(const Py::Tuple& args)
{
    ...
    auto tupleToColor = [](PyObject* col) {
        ...
        // Extract colors from the Python tuple and clamp to valid range.
        auto red = std::clamp(static_cast<float>(Py::Float(tuple[0])), 0.0F, 1.0F);
        auto green = std::clamp(static_cast<float>(Py::Float(tuple[1])), 0.0F, 1.0F);
        auto blue = std::clamp(static_cast<float>(Py::Float(tuple[2])), 0.0F, 1.0F);

        return QColor::fromRgbF(red, green, blue);
    };

    try {
        QColor from_color = tupleToColor(col1);
        QColor to_color = tupleToColor(col2);
        std::optional<QColor> mid_color;
        if (col3) {
            mid_color = tupleToColor(col3);
        }

        _viewer->setGradientBackgroundColor(from_color, to_color, mid_color);
        _viewer->redraw();
        return Py::None();
    }
    catch (const Base::Exception& e) {
...
Py::Object View3DInventorViewerPy::setBackgroundColor(const Py::Tuple& args)
{
    ...
    try {
        // Clamp colors the valid range.
        red = std::clamp(red, 0.0F, 1.0F);
        green = std::clamp(green, 0.0F, 1.0F);
        blue = std::clamp(blue, 0.0F, 1.0F);

        QColor background_color = QColor::fromRgbF(red, green, blue);

        _viewer->setBackgroundColor(background_color);
        _viewer->redraw();
        return Py::None();
    }
    catch (const Base::Exception& e) {
